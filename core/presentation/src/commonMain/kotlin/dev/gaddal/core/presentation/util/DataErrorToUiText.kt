...
/**
 * Converts a `DataError` instance to a corresponding `UiText` representation.
 *
 * Maps specific error types within `DataError` (both local and remote)
 * to predefined `UiText.Resource` values for user interface display.
 *
 * @return A `UiText` instance representing the corresponding user-friendly error message or text.
 */
